...
public class EvaluationResultsController : ControllerBase
{
    ...
    public EvaluationResultsController(AppDbContext db, TniGenerationService tniGenerator)
    {
        _db = db;
    ...
    public async Task<ActionResult<EvaluationResultDto>> Create([FromBody] CreateEvaluationResultDto dto)
    {
        ...
        // A field "scores below threshold" when:
        //   binary (MaxRating=1) → score is 0 (FAIL)
        //   rating (MaxRating>1) → score ≤ floor(MaxRating * 0.4), e.g. ≤ 2 for 1-5 scale
        var failedFields = result.Scores
            .Where(s => s.Field != null && s.NumericValue.HasValue &&
                        (s.Field.MaxRating == 1
                            ? s.NumericValue.Value == 0
                            : s.NumericValue.Value <= Math.Floor(s.Field.MaxRating * 0.4)))
            .ToList();
        ...
        if (failedFields.Count > 0 && !string.IsNullOrWhiteSpace(dto.AgentName))
        {
            ...
            // Fire-and-forget LLM content generation — errors are logged, not surfaced to caller
            _ = Task.Run(async () =>
            {
                try
                {
                    var (content, json) = await _tniGenerator.GenerateAsync(
                        targetAreas, dto.AgentName, result.Form?.Name ?? "QA Form");
                ...
                {
                    // Log but do not fail the evaluation result creation
                    var logger = HttpContext.RequestServices.GetService<ILogger<EvaluationResultsController>>();
                    logger?.LogWarning(ex, "Auto-TNI LLM generation failed for plan linked to result {ResultId}", result.Id);
                }
